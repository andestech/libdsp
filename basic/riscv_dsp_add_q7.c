...
void riscv_dsp_add_q7(q7_t * FUNC_RESTRICT src1, q7_t * FUNC_RESTRICT src2, q7_t * FUNC_RESTRICT dst, uint32_t size)
{

    while (size != 0u)
    {
        /* y = x1 + x2 and then saturated in Q7 range. */
        *dst++ = (q7_t) NDS_ISA_SATS((q15_t) (*src1++) + (*src2++), 8u);
        size--;
    }
