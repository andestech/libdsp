...
void riscv_dsp_offset_q31(q31_t * FUNC_RESTRICT src, q31_t offset, q31_t * FUNC_RESTRICT dst, uint32_t size)
{
    while (size != 0u)
    {
        /* y = x + offset then saturated in Q31 range */
    *dst++ = (q31_t)NDS_ISA_KADDW((*src++) , offset);
        size--;
    }
}
