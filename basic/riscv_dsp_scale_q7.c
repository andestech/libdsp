 ...
 * @return none.
 *
 * The inputs are multiplied to yield a 2.14 output and then are shift with
 * saturation to 1.7 formats.
 */
/* function description */
void riscv_dsp_scale_q7(q7_t * FUNC_RESTRICT src, q7_t scalefract, int8_t shift, q7_t * FUNC_RESTRICT dst,
                        uint32_t size)
{
    ...
    while (size != 0u)
    {
        ...
        *dst++ = (q7_t) (NDS_ISA_SATS(((q31_t) * src++ * scalefract) >> shift1, 8u));
        size--;
    }
}
