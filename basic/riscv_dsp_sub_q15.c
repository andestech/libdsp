...
void riscv_dsp_sub_q15(q15_t * FUNC_RESTRICT src1, q15_t * FUNC_RESTRICT src2, q15_t * FUNC_RESTRICT dst, uint32_t size)
{

    while (size != 0u)
    {
        /* y = x1 - x2 and then saturated in Q15 range. */
        *dst++ = (q15_t) NDS_ISA_KSUBH(*src1++, *src2++);
        size--;
    }
