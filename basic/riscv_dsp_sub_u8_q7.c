...
void riscv_dsp_sub_u8_q7(uint8_t * FUNC_RESTRICT src1, uint8_t * FUNC_RESTRICT src2, q7_t * FUNC_RESTRICT dst, uint32_t size)
{

    while (size != 0u)
    {
        *dst++ = (q7_t) NDS_ISA_SATS((uint16_t) (*src1++) - (*src2++), 8u);
        size--;
    }
