...
void riscv_dsp_bq_df1_32x64_q31(const riscv_dsp_bq_df1_32x64_q31_t * FUNC_RESTRICT instance,
                                q31_t * FUNC_RESTRICT src, q31_t * FUNC_RESTRICT dst, uint32_t size)
{
    q31_t *in = src;
    q31_t *out = dst;
    q63_t *state = instance->state;
    ...
    int32_t shift = (int32_t) instance->shift + 1;
    q31_t acc_l, acc_h;
    ...
    while (stage != 0u)
    {
        ...
        while (i != 0u)
        {
            ...
            acc += (q63_t) b1 * xn1;
            acc += (q63_t) b2 * xn2;
            acc += dsp_mul_64x32(yn1, a1);
            acc += dsp_mul_64x32(yn2, a2);
            xn2 = xn1;
            xn1 = xn;
            yn2 = yn1;
            yn1 = acc << shift;
