                        ...
                        uint32_t size)
{
    ...
    while (size != 0u)
    {
        ...
        *err++ = e;
        ...
        postshift = dsp_recip_q31(energy + DELTA_Q31, &onebyenergy);

        errormu = (q31_t) (((q63_t) e * mu) >> 31);

        w = dsp_truncate_q63_to_q31(((q63_t) errormu * onebyenergy) >> (31 - postshift));
        ...
        while (tapcnt != 0u)
        {
            ...
            *pb = dsp_truncate_q63_to_q31((q63_t) * pb + (coef << 1u));
            pb++;
            tapcnt--;
        }
