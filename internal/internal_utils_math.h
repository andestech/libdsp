...
static inline int32_t _dsp_log2_16b(uint16_t v)
{
    uint32_t r, shift;
...
static inline int32_t _dsp_log2_32b(uint32_t v)
{
    uint32_t r, shift;
...
static inline uint32_t _dsp_ghb_16b(q15_t data)
{
    uint32_t count;
    ...
    if (data != 0)
    {
        count = _dsp_log2_16b(data) + 1;
    }
    return count;
}

// Get the index of Highest Bit for 32-bit value
static inline uint32_t _dsp_ghb_32b(q31_t data)
{
    uint32_t count;
    ...
    if (data != 0)
    {
        count = _dsp_log2_32b(data) + 1;
    }
    return count;
}

// Count Leading Zero
static inline uint32_t _dsp_clz(q31_t data)
{
    uint32_t count;
    count = 32 - _dsp_ghb_32b(data);
    return count;
}

// Absolute q31 value
//Use definition from internal header files rather than re-defined ABS.
#define _dsp_abs(X)    NDS_ISA_ABS(X)
...
static inline q31_t _dsp_div_u64_u32(uint64_t src1, uint32_t src2)
#else
static inline q31_t _dsp_div_u64_u32(uint64_t src1, uint32_t src2, uint32_t * r)
#endif
{
    uint32_t n1, n0, cnt;
...
#ifndef ENA_DIV_REMAINDER
static inline q31_t _dsp_div_s64_u32(int64_t src1, uint32_t src2)
#else
static inline q31_t _dsp_div_s64_u32(int64_t src1, uint32_t src2, int32_t * r)
#endif
{
    int32_t sgn = 0, ret;
    ...
    ret = _dsp_div_u64_u32(src1, src2);
#else
    ret = _dsp_div_u64_u32(src1, src2, r);
#endif
    if (sgn)
    {
        ret = -ret;
...
#if defined (__riscv_zfh)
static inline float16_t taylor_poly_exp_f16(float16_t x)
{
...
static inline float16_t _exp_f16(float16_t x)
{
    // Perform range reduction [-log(2),log(2)]
    ...
    // Polynomial Approximation
    _union16_t poly;
    poly.f16 = taylor_poly_exp_f16(val);
...
static inline float16_t taylor_poly_log_f16(float16_t x)
{
...
static inline float16_t _log_f16(float16_t x)
{
    // Extract exponent
    _union16_t val, tmp;
    val.f16 = x;
    ...
    // Polynomial Approximation
    float16_t out = taylor_poly_log_f16(val.f16);
...
static inline float16_t _sin_f16(float16_t x)
{
    // algorithm from compute library
    _union16_t c_v;
...
#if defined (__riscv_zfh)
static inline float16_t _cos_f16(float16_t x)
{
