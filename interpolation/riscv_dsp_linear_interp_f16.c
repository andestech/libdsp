...
int riscv_dsp_linear_interp_f16(riscv_dsp_linear_interp_f16_t * FUNC_RESTRICT instance, float16_t * FUNC_RESTRICT src, float16_t * FUNC_RESTRICT dst, uint32_t size)
{
    float16_t out, in_x;
    float16_t x0, x1, y0, y1;
    uint32_t idx;
    const float16_t *py_tbl = instance->tbl_data;
    float16_t init_x = instance->first_x;
    float16_t xspac = instance->x_spacing;
    float16_t rec_xspac = 1.0 / xspac;
