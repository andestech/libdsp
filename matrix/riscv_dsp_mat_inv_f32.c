...
riscv_dsp_mat_inv_f32(float32_t * FUNC_RESTRICT src, float32_t * FUNC_RESTRICT dst, uint32_t size)
{
    float32_t *in = src;
    float32_t *pt1, *pt2, *pt3, *pt4;
    float32_t *diagsrc, *rtsrc, *diagdst, *rtdst;
    float32_t tmp, tmp1 = 0.0f, tmp2;
    uint32_t i, j, k, l;
    ...
    memset(pt2, 0, sizeof(float32_t)*size * size);
    for (i = 0; i < rowcnt; i++)
    {
        *pt2 = 1.0f;
        pt2 += (rowcnt + 1);
