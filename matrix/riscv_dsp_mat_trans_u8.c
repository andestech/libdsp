...
void riscv_dsp_mat_trans_u8(const uint8_t * FUNC_RESTRICT src, uint8_t * FUNC_RESTRICT dst, uint32_t row, uint32_t col)
{
#ifdef NDS_CPU_64
    u32_t i, j;
    uuni_t value;
    ...
    const uint8_t *tmp_src2 = src;
    uint8_t *tmp_dst2 = dst;
    ...
    for(i=0; i<row; i++)
    {
        const uint8_t *tmp_src = tmp_src2;
        uint8_t *tmp_dst = tmp_dst2;

        for(j=0; j<(col/SIMD_NUM_Q7); j++)
        {
            value = *(uuni_t*)tmp_src;
            *tmp_dst = value.b8.b0;
            tmp_dst += row;
            *tmp_dst = value.b8.b1;
            tmp_dst += row;
