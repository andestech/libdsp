...
ENA_TEST_FFT=1
if [[ $EXTRA_FLAGS == *"-DONLY_GEN_ADX"* ]]; then
    ENA_ONLY_GEN_ADX=1
    ENA_TEST_FFT=0
else
    ENA_ONLY_GEN_ADX=0
...
C_FLAGS="-O3 -DRES_LOGN=10 -Wall -Werror -Wno-narrowing"
...
function run_and_check {
        ...
        echo "============== test $BIN_FILE =============="
        if [ "$ENA_ONLY_GEN_ADX" == "1" ]; then
            echo "${CC} ${INCLUDE} ${CFLAGS} -DGETENV_SRCDIR=\"../testsuite/$SRC_FOLDER\" -DGET_TESTDIR=../testsuite/$SRC_FOLDER -o ${BIN_FILE} ${SRC_DIR}/testsuite/${SRC_FOLDER}/${BIN_FILE}.c ${LIB_PATH} -lm ${EXTRA_FLAGS}"
            ${CC} ${INCLUDE} ${CFLAGS} -DGETENV_SRCDIR=\"../testsuite/$SRC_FOLDER\" -DGET_TESTDIR=../testsuite/$SRC_FOLDER -o ${BIN_FILE} ${SRC_DIR}/testsuite/${SRC_FOLDER}/${BIN_FILE}.c ${LIB_PATH} -lm ${EXTRA_FLAGS}

            echo "gen ${BIN_FILE} done"
            echo "======================="
        else
            echo "${CC} ${INCLUDE} ${CFLAGS} -DGETENV_SRCDIR=\"${SRC_DIR}/testsuite/$SRC_FOLDER\" -DGET_TESTDIR=${SRC_DIR}/testsuite/$SRC_FOLDER -o ${BIN_FILE} ${SRC_DIR}/testsuite/${SRC_FOLDER}/${BIN_FILE}.c ${LIB_PATH} -lm ${EXTRA_FLAGS}"
            ${CC} ${INCLUDE} ${CFLAGS} -DGETENV_SRCDIR=\"${SRC_DIR}/testsuite/$SRC_FOLDER\" -DGET_TESTDIR=${SRC_DIR}/testsuite/$SRC_FOLDER -o ${BIN_FILE} ${SRC_DIR}/testsuite/${SRC_FOLDER}/${BIN_FILE}.c ${LIB_PATH} -lm ${EXTRA_FLAGS}
            pwd
            ...
            ${OBJ} -S ${BIN_FILE} > ${BIN_FILE}.objdump
            echo ""
        fi
}
