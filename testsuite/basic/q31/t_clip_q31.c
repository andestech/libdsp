...
int main(void)
{
    ...
    q31_t high = 0x4fffffff;
    q31_t low = 0xa0000000;
    ...
    sprintf (path_buffer, "%s/golden_clip_q31.bin", srcdir);
    ...
    riscv_dsp_clip_q31(inputvec, outputvec, low, high, blocksize);
#if 0
    FILE *fptr_golden = NULL;
    sprintf (path_buffer, "%s/golden_clip_q31_out.bin", srcdir);
    fptr_golden = fopen(path_buffer, "wb");
    fwrite(outputvec, sizeof(q31_t), blocksize, fptr_golden);
    fclose(fptr_golden);
#endif
