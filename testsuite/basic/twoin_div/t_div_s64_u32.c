...
int main(void)
{
    ...
    sprintf (path_buffer, "%s/input_q63.bin", srcdir);
    fptr = fopen(path_buffer, "r");
    if (fptr == NULL)
        return -1;

    sprintf (path_buffer, "%s/input_q63_u32.bin", srcdir);
    f2ptr = fopen(path_buffer, "r");
    if (fptr == NULL)
        return -1;

    sprintf (path_buffer, "%s/golden_div_s64.bin", srcdir);
    ...
    {
        test_Fread(&inputAvec, sizeof(int64_t), 1, fptr);
        test_Fread(&inputBvec, sizeof(uint32_t), 1, f2ptr);
        outputvec[i] = riscv_dsp_div_s64_u32(inputAvec, inputBvec);
    }
