...
int main(void)
{
    ...
    sprintf (path_buffer, "%s/golden_or_u16.bin", srcdir);
    ...
    //dump_u16("roromly generated input array", inputvec, blocksize);
    test_Fread(goldenvec, sizeof(u16_t), blocksize, gptr);
    fclose(fptr);
    fclose(fptr2);
    fclose(gptr);

    riscv_dsp_or_u16(input1, input2, outputvec, blocksize);
#ifdef WRITE_GOLDEN
    FILE *fptr_golden = NULL;
    sprintf (path_buffer, "%s/golden_or_u16_out.bin", srcdir);
