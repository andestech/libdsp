...
int main(void)
{
    ...
    sprintf (path_buffer, "%s/golden_or_u32.bin", srcdir);
    ...
    //dump_u32("roromly generated input array", inputvec, blocksize);
    test_Fread(goldenvec, sizeof(u32_t), blocksize, gptr);
    fclose(fptr);
    fclose(fptr2);
    fclose(gptr);

    riscv_dsp_or_u32(input1, input2, outputvec, blocksize);
#ifdef WRITE_GOLDEN
    FILE *fptr_golden = NULL;
    sprintf (path_buffer, "%s/golden_or_u32_out.bin", srcdir);
