...
int main(void)
{
    ...
    u32_t input1[blocksize], input2[blocksize];
    ...
    sprintf (path_buffer, "%s/golden_xor_u32.bin", srcdir);
    gptr = fopen(path_buffer, "r");
    if (gptr == NULL)
        return -1;

    test_Fread(input1, sizeof(u32_t), blocksize, fptr);
    test_Fread(input2, sizeof(u32_t), blocksize, fptr2);
    //dump_u32("rxoromly generated input array", inputvec, blocksize);
    test_Fread(goldenvec, sizeof(u32_t), blocksize, gptr);
    fclose(fptr);
    fclose(fptr2);
    fclose(gptr);

    riscv_dsp_xor_u32(input1, input2, outputvec, blocksize);
#ifdef WRITE_GOLDEN
    FILE *fptr_golden = NULL;
    sprintf (path_buffer, "%s/golden_xor_u32_out.bin", srcdir);
