...
int main(void)
{
    ...
    sprintf (path_buffer, "%s/golden_and_u8.bin", srcdir);
    ...
    //dump_u8("randomly generated input array", inputvec, blocksize);
    test_Fread(goldenvec, sizeof(u8_t), blocksize, gptr);
    fclose(fptr);
    fclose(fptr2);
    fclose(gptr);

    riscv_dsp_and_u8(input1, input2, outputvec, blocksize);
#ifdef WRITE_GOLDEN
    FILE *fptr_golden = NULL;
    sprintf (path_buffer, "%s/golden_and_u8_out.bin", srcdir);
