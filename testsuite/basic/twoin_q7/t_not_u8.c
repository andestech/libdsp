...
int main(void)
{
    ...
    sprintf (path_buffer, "%s/golden_not_u8.bin", srcdir);
    gptr = fopen(path_buffer, "r");
    if (gptr == NULL)
        return -1;

    test_Fread(input1, sizeof(u8_t), blocksize, fptr);
    //dump_u8("rnotomly generated input array", inputvec, blocksize);
    test_Fread(goldenvec, sizeof(u8_t), blocksize, gptr);
    fclose(fptr);
    fclose(gptr);

    riscv_dsp_not_u8(input1, outputvec, blocksize);
#ifdef WRITE_GOLDEN
    FILE *fptr_golden = NULL;
    sprintf (path_buffer, "%s/golden_not_u8_out.bin", srcdir);
