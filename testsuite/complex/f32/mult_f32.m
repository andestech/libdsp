...
ofid =fopen('golden_cmplx_mult_f32.bin', 'wb');
...
for mm = 1 : datasize/(2*numSamples)
    ...
    for n = 1 : numSamples
%         parta = inputa((2*n)-1)*inputb((2*n)-1);
%         partb = inputa((2*n))*inputb((2*n));
%         partc = inputa((2*n)-1)*inputb((2*n));
%         partd = inputa((2*n))*inputb((2*n)-1);
