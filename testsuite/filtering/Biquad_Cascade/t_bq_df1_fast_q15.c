...
int main(void)
{
    ...
    sprintf (path_buffer, "%s/input_fast_q15.bin", srcdir);
    fptr = fopen(path_buffer, "r");
    if (fptr == NULL)
        return -1;

    sprintf (path_buffer, "%s/golden_bq_df1_fast_q15.bin", srcdir);
    gptr = fopen(path_buffer, "r");
    if (gptr == NULL)
        return -1;

    sprintf (path_buffer, "%s/golden_bq_df1_fast_state_q15.bin", srcdir);
    ...
    riscv_dsp_bq_df1_fast_q15(&Si, input, output, bsize);
    q15_t maxdiff = dump2_q15(golden, output, bsize);
    if (maxdiff > 0x0002)
    {
        printf("accuracy fail\n");
        return -1;
    }
    maxdiff = dump2_q15(golden_state, state, 4 * stage);
    if (maxdiff > 0x0001)
    {
