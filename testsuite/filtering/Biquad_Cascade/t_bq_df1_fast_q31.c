...
int main(void)
{
    ...
    q31_t *state = (q31_t *)calloc(4 * stage, sizeof(q31_t));
    ...
    riscv_dsp_bq_df1_q31_t Si = {stage, state, coe, shift};

    q31_t input[bsize], output[bsize], golden[bsize], golden_state[4 * stage];
    ...
    sprintf (path_buffer, "%s/golden_biquad_cascade_df1_state_q31.bin", srcdir);
    ...
    test_Fread(&golden_state, sizeof(q31_t), 4 * stage, g2ptr);

    riscv_dsp_bq_df1_fast_q31(&Si, input, output, bsize);
    q31_t maxdiff = dump2_q31(golden, output, bsize);
    if (maxdiff > 0x00000b00)
    {
        printf("accuracy fail\n");
        return -1;
    }
    maxdiff = dump2_q31(golden_state, state, 4 * stage);
    if (maxdiff > 0x00000700)
    {
        printf("state checking fail\n");
        return -1;
    }
    ...
    dump_rmsd(golden_f32, output_f32, bsize);
    return 0;
}
