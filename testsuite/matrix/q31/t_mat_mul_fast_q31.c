...
int main(void)
{
    ...
    sprintf (path_buffer, "%s/input_q31_fast_a.bin", srcdir);
    fptr = fopen(path_buffer, "r");
    if (fptr == NULL)
        return -1;

    sprintf (path_buffer, "%s/input_q31_fast_b.bin", srcdir);
    gptr = fopen(path_buffer, "r");
    if (gptr == NULL)
        return -1;

    sprintf (path_buffer, "%s/golden_mat_mul_fast_q31.bin", srcdir);
    ...
    riscv_dsp_mat_mul_fast_q31(pInA, pInB, output, MATRIX_M, MATRIX_N, MATRIX_P);
