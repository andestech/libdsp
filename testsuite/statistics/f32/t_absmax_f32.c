...
int main(void)
{
    ...
    sprintf (path_buffer, "%s/golden_absmax_f32_index.bin", srcdir);
    ...
    for (i = 0; i < loop; i++)
    {
        ...
        out_max[i] = riscv_dsp_absmax_f32(input, 0, &out_index); //test the function when size is 0
        out_max[i] = riscv_dsp_absmax_f32(input, blocksize, &out_index);
