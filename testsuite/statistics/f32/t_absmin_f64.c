...
int main(void)
{
    ...
    sprintf (path_buffer, "%s/golden_absmin_f64_absmin.bin", srcdir);
    gptr = fopen(path_buffer, "r");
    if (gptr == NULL)
        return -1;

    sprintf (path_buffer, "%s/golden_absmin_f64_index.bin", srcdir);
    ...
    for (i = 0; i < loop; i++)
    {
        ...
        out_min[i] = riscv_dsp_absmin_f64(input, 0, &out_index); //test the function when size is 0
        out_min[i] = riscv_dsp_absmin_f64(input, blocksize, &out_index);
