...
int main(void)
{
    ...
    sprintf (path_buffer, "%s/golden_min_f32_min.bin", srcdir);
    gptr = fopen(path_buffer, "r");
    if (gptr == NULL)
        return -1;

    sprintf (path_buffer, "%s/golden_min_f32_index.bin", srcdir);
    ...
    for (i = 0; i < loop; i++)
    {
        ...
        out_min[i] = riscv_dsp_min_f32(input, 0, &out_index);
        out_min[i] = riscv_dsp_min_f32(input, blocksize, &out_index);
