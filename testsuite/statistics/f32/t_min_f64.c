...
int main(void)
{
    ...
    sprintf (path_buffer, "%s/golden_min_f64_index.bin", srcdir);
    ...
    for (i = 0; i < loop; i++)
    {
        ...
        out_min[i] = riscv_dsp_min_f64(input, 0, &out_index);
        out_min[i] = riscv_dsp_min_f64(input, blocksize, &out_index);
