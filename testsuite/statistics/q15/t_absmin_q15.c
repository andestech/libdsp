...
int main(void)
{
    ...
    sprintf (path_buffer, "%s/golden_absmin_q15_absmin.bin", srcdir);
    gptr = fopen(path_buffer, "r");
    if (gptr == NULL)
        return -1;

    sprintf (path_buffer, "%s/golden_absmin_q15_index.bin", srcdir);
    ...
    for (i = 0; i < loop; i++)
    {
        ...
        out_min[i] = riscv_dsp_absmin_q15(input, 0, &out_index);
        out_min[i] = riscv_dsp_absmin_q15(input, blocksize, &out_index);
