...
int main(void)
{
    ...
    u16_t out_min[loop], gold_min[loop];
    ...
    sprintf (path_buffer, "%s/golden_min_u16_index.bin", srcdir);
    ...
    for (i = 0; i < loop; i++)
    {
        ...
        out_min[i] = riscv_dsp_min_u16(input, 0, &out_index);
        out_min[i] = riscv_dsp_min_u16(input, blocksize, &out_index);
    ...
    test_Fread(&gold_min, sizeof(u16_t), loop, gptr);
    dump2_u16(gold_min, out_min, loop);
    ...
    _dsp_convert_u16_f32(gold_min, golden_f32, loop);
    _dsp_convert_u16_f32(out_min, output_f32, loop);
