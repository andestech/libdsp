...
int main(void)
{
    ...
    sprintf (path_buffer, "%s/golden_max_u32_index.bin", srcdir);
    ...
    for (i = 0; i < loop; i++)
    {
        ...
        out_max[i] = riscv_dsp_max_u32(input, 0, &out_index);
        out_max[i] = riscv_dsp_max_u32(input, blocksize, &out_index);
