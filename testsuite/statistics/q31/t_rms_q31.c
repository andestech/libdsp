...
int main(void)
{
    ...
    sprintf (path_buffer, "%s/input_rms_q31.bin", srcdir);
    fptr = fopen(path_buffer, "r");
    if (fptr == NULL)
        return -1;

    sprintf (path_buffer, "%s/golden_rms_q31.bin", srcdir);
    ...
    for (i = 0; i < loop; i++)
    {
        ...
        outputda[i] = riscv_dsp_rms_q31(input, blocksize);
    }
