...
int main(void)
{
    ...
    sprintf (path_buffer, "%s/golden_max_q7_max.bin", srcdir);
    gptr = fopen(path_buffer, "r");
    if (gptr == NULL)
        return -1;

    sprintf (path_buffer, "%s/golden_max_q7_index.bin", srcdir);
    ...
    for (i = 0; i < loop; i++)
    {
        ...
        out_max[i] = riscv_dsp_max_q7(input, 0, &out_index);
        out_max[i] = riscv_dsp_max_q7(input, blocksize, &out_index);
