...
int main(void)
{
    ...
    riscv_dsp_svm_linear_f32_t Si = {NUMVEC, NUMDIM, INTERCEPT, dualcoe, vec, pclass};
    ...
    sprintf (path_buffer, "%s/golden_svm_linear_predict_f32.bin", srcdir);
    ...
    for (i = 0; i < loop; i++)
    {
        ...
        riscv_dsp_svm_linear_est_f32(&Si, input, &outputda[i]);
    }
#ifdef PRINT_DETAIL
