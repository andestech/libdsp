...
int main(void)
{
    ...
    int i;
    FILE *fptr;
    fptr = fopen("golden_cfft_f32.bin", "r");
    FILE *gptr;
    gptr = fopen("golden_cifft_f32.bin", "r");
    float32_t NRMSD;
    FILE *f2ptr;
    f2ptr = fopen("input_cfft_rand_f32.bin", "r");
    FILE *g2ptr;
    g2ptr = fopen("golden_cfft_rand_f32.bin", "r");
    FILE *g3ptr;
    g3ptr = fopen("golden_cifft_rand_f32.bin", "r");
    ...
    {
        buf[2 * i] = buf2[2 * i] = (convert_float_to_q31(test_fft_Input[i])) >> 1;
        buf[2 * i + 1] = buf2[2 * i + 1] = (convert_float_to_q31(test_fft_Input[i])) >> 1;
    }
    dump_q31("randomly generated input array", buf, 2 * TRANSFORM_SAMPLES);
    ...
    // Sine Tone case
    riscv_dsp_cfft_q31(buf, TRANSFORM_SAMPLES_LOG);
    dump_q31("after cFFT (Forward)", buf, 2 * TRANSFORM_SAMPLES);

    /* check dct with golden */
    printf("--------------\n");
    NRMSD = dump2_goldendiff_q31("after CFFT_RD2 (Forward)", golden_buf, buf, 2 * TRANSFORM_SAMPLES, 1);
    printf("CFFT_RD2 out scaleup by %d\n", (2 << TRANSFORM_SAMPLES_LOG));
    LOG("--------------\n");

    if (NRMSD > 0.0000001)
    {
        printf("NRMSD = %2.8f > 0.0000001, CFFT_RD2 FAIL\n", NRMSD);
        return -1;
    }
    // Random case
    riscv_dsp_cfft_q31(ran_q31_in, TRANSFORM_SAMPLES_LOG);
    ...
    // Sine Tone case
    riscv_dsp_cifft_q31(buf, TRANSFORM_SAMPLES_LOG);
    dump_q31("after cIFFT (Backward) ", buf, 2 * TRANSFORM_SAMPLES);

    /* check idct with golden */
    NRMSD = dump2_inv_goldendiff_q31("after CFFT_RD2 (Backward)", golden_ibuf, buf, 2 * TRANSFORM_SAMPLES);
    ...
    if (TRANSFORM_SAMPLES_LOG <= 10)
    {
        if (NRMSD >  0.0000001)
        {
            printf("NRMSD = %2.8f > 0.0000001, CIFFT_RD2 FAIL\n", NRMSD);
            return -1;
        }
    }
    // Random case
    riscv_dsp_cifft_q31(ran_q31_in, TRANSFORM_SAMPLES_LOG);
    NRMSD = dump2_inv_goldendiff2_q31("after CIFFT_RD4 random case (Backward)", ran_buf_cifft_golden, ran_q31_in, 2 * TRANSFORM_SAMPLES);

    riscv_dsp_cifft_q31(buf3, TRANSFORM_SAMPLES_LOG);
    dump_q31("after cIFFT (Forward + Backward) ", buf3, 2 * TRANSFORM_SAMPLES);

    /* check idct with golden */
    NRMSD = dump2_inv_goldendiff_q31("after CFFT_RD2 (Forward + Backward)", golden_ibuf, buf3, 2 * TRANSFORM_SAMPLES);
    printf("CIFFT_RD2 out scaleup by %d\n", 2);
    LOG("--------------\n");
