...
{
    FILE *fptr;
    fptr = fopen("golden_cfft_rd2_f32.bin", "w");
    FILE *gptr;
    gptr = fopen("golden_cifft_rd2_f32.bin", "w");
    FILE *f2ptr;
    f2ptr = fopen("input_cfft_rd2_rand_f32.bin", "w");
    FILE *g2ptr;
    g2ptr = fopen("golden_cfft_rd2_rand_f32.bin", "w");
    FILE *g3ptr;
    g3ptr = fopen("golden_cifft_rd2_rand_f32.bin", "w");
    ...
    // Sine Tone case
    riscv_dsp_cfft_rd2_f32(buf, TRANSFORM_SAMPLES_LOG);
    dump_f32("after cFFT (Forward)", buf, 2 * TRANSFORM_SAMPLES);
    // Random case
    riscv_dsp_cfft_rd2_f32(ran_buf_in, TRANSFORM_SAMPLES_LOG);
    ...
    // Sine Tone case
    riscv_dsp_cifft_rd2_f32(cfft_golden, TRANSFORM_SAMPLES_LOG);
    dump_f32("after cIFFT (Backward)", cfft_golden, 2 * TRANSFORM_SAMPLES);
    // Random case
    riscv_dsp_cifft_rd2_f32(ran_buf_in, TRANSFORM_SAMPLES_LOG);
    ...
    riscv_dsp_cifft_rd2_f32(buf, TRANSFORM_SAMPLES_LOG);
