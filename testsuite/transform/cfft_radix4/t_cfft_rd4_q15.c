...
int main(void)
{
    ...
    {
        buf[2 * i] = buf2[2 * i] = (convert_float_to_q15(test_fft_Input[i]));
        buf[2 * i + 1] = buf2[2 * i + 1] = (convert_float_to_q15(test_fft_Input[i]));
    }
    for (i = 0; i < 2*TRANSFORM_SAMPLES; i++)
    {
        ran_q15_in[i] = (convert_float_to_q15(ran_buf_in[i]));
    }
    dump_q15("randomly generated input array", ran_q15_in, 2 * TRANSFORM_SAMPLES);

    // Sine Tone case
    riscv_dsp_cfft_rd4_q15(buf, TRANSFORM_SAMPLES_LOG);
    //dump_q15("after cFFT (Forward)", buf, 2 * TRANSFORM_SAMPLES);
    /* check dct with golden */
    printf("--------------\n");
    NRMSD = dump2_goldendiff2_q15("after CFFT_RD4 sinetone case (Forward)", golden_buf, buf, 2 * TRANSFORM_SAMPLES, 1);
    printf("CFFT_RD4 out scaleup by %d\n", (2 << TRANSFORM_SAMPLES_LOG));
    LOG("--------------\n");

    if (NRMSD > 0.00007)
    {
        printf("NRMSD = %2.8f > 0.00007, CFFT_RD4 FAIL\n", NRMSD);
        return -1;
    }

    // Random case
    riscv_dsp_cfft_rd4_q15(ran_q15_in, TRANSFORM_SAMPLES_LOG);
    ...
    // Sine Tone case
    riscv_dsp_cifft_rd4_q15(buf, TRANSFORM_SAMPLES_LOG);
    //dump_q15("after cIFFT (Backward) ", buf, 2 * TRANSFORM_SAMPLES);

    /* check idct with golden */
    NRMSD = dump2_inv_goldendiff_q15("after CIFFT_RD4 sinetone case (Backward)", golden_ibuf, buf, 2 * TRANSFORM_SAMPLES);
    ...
    // Random case
    riscv_dsp_cifft_rd4_q15(ran_q15_in, TRANSFORM_SAMPLES_LOG);
    ...
    riscv_dsp_cifft_rd4_q15(buf3, TRANSFORM_SAMPLES_LOG);
    //dump_q15("after cIFFT (Forward + Backward) ", buf3, 2 * TRANSFORM_SAMPLES);

    /* check cifft with golden */
    NRMSD = dump2_inv_goldendiff_q15("after CIFFT_RD4 (Forward + Backward)", golden_ibuf, buf3, 2 * TRANSFORM_SAMPLES);
    printf("CIFFT_RD4 out scaleup by % d\n", 2);
    LOG("--------------\n");
...
#if 0
    // check cifft for bitexact
    if (TRANSFORM_SAMPLES_LOG <= 10)
    {
        int32_t check;
        check = riscv_dsp_sumdiff_q15(buf3, 2 * TRANSFORM_SAMPLES, CIFFT_SUMOUT);
        if (check != 0)
        {
            printf("cifft bit exact checking fail\n");
            return -1;
        }
        else
        {
            printf("cifft bit exact checking pass\n");
        }
    }
#endif
    if (TRANSFORM_SAMPLES_LOG <= 10)
    {
        ...
        {
            printf("NRMSD = %2.8f > 0.005, CIFFT_RD2 FAIL\n", NRMSD);
            return -1;
        }
    }
    else
