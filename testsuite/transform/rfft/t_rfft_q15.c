...
int main(void)
{
    ...
    q15_t ran_q15_in[TRANSFORM_SAMPLES];
    ...
    {
        buf[i] = buf2[i] = (convert_float_to_q15(test_fft_Input[i]));
        ran_q15_in[i] = (convert_float_to_q15(ran_buf_in[i]));
    }
    dump_q15("randomly generated input array", buf, TRANSFORM_SAMPLES);

    // Sine Tone case
    riscv_dsp_rfft_q15(buf, TRANSFORM_SAMPLES_LOG);
    dump_q15("after rFFT", buf, TRANSFORM_SAMPLES);

    /* check dct with golden */
    printf("--------------\n");
    printf("after rFFT (Forward), ");
    NRMSD = dump2_goldendiff2_q15("after RFFT", golden_dct, buf, TRANSFORM_SAMPLES, 0);
    ...
    if (NRMSD > 0.0003)
    {
        printf("NRMSD = %2.8f > 0.0003, RFFT FAIL\n", NRMSD);
        return -1;
    }
    // Random case
    riscv_dsp_rfft_q15(ran_q15_in, TRANSFORM_SAMPLES_LOG);
    NRMSD = dump2_goldendiff2_q15("after RFFT", ran_buf_rfft_golden, ran_q15_in, TRANSFORM_SAMPLES, 0);
    if (NRMSD > 0.0003)
    {
    ...
    // Sine Tone case
    riscv_dsp_rifft_q15(buf, TRANSFORM_SAMPLES_LOG);
    /* check idct with golden */
    printf("after rFFT (Backward), ");
    NRMSD = dump2_inv_goldendiff_q15("after RIFFT", golden_idct, buf, TRANSFORM_SAMPLES);
    ...
    if (TRANSFORM_SAMPLES_LOG <= 10)
    {
        if (NRMSD >  0.0002)
        {
            printf("NRMSD = %2.8f > 0.0002, RIFFT_RD2 FAIL\n", NRMSD);
            return -1;
        }
    }
    for (i = 0; i < TRANSFORM_SAMPLES; i++)
    ...
    // Random case
    riscv_dsp_rifft_q15(ran_q15_in, TRANSFORM_SAMPLES_LOG);
    NRMSD = dump2_inv_goldendiff_q15("after RIFFT", ran_buf_rifft_golden, ran_q15_in, TRANSFORM_SAMPLES);
    if (TRANSFORM_SAMPLES_LOG <= 10)
    {
        if (NRMSD >  0.007)
        {
            printf("check randon\n");
            printf("NRMSD = %2.8f > 0.007, RIFFT_RD2 FAIL\n", NRMSD);
            return -1;
        }
    }
    else
    {
        if ((NRMSD / TRANSFORM_SAMPLES_LOG) >  0.008)
        {
            printf("check randon\n");
            printf("NRMSD = %2.8f > 0.008, RIFFT_RD2 FAIL\n", NRMSD);
            return -1;
        }
    }
    for (i = 0; i < TRANSFORM_SAMPLES; i++)
    ...
    riscv_dsp_rifft_q15(buf3, TRANSFORM_SAMPLES_LOG);
    printf("after rFFT (Forward + Backward), ");
    NRMSD = dump2_inv_goldendiff_q15("after RIFFT", golden_idct, buf3, TRANSFORM_SAMPLES);
    printf("RIFFT out scaleup by %d\n", 2);
    LOG("--------------\n");
    ...
    if (TRANSFORM_SAMPLES_LOG <= 10)
    {
        ...
        {
            printf("NRMSD = %2.8f > 0.003, RIFFT_RD2 FAIL\n", NRMSD);
            return -1;
        }
    }
    else
