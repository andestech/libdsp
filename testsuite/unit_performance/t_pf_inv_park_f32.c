...
float32_t sinval[BUF_SIZE_F32] FUNC_ATTR_ALIGN;
float32_t cosval[BUF_SIZE_F32] FUNC_ATTR_ALIGN;
...
int main(void)
{
    ...
    for (j = 0; j < TEST_LOOP_F32; j++)
    {
        ...
        for (i = 0; i < BUF_SIZE_F32; i++)
        {
            riscv_dsp_inv_park_f32(srcf32_1[i],  srcf32_2[i], &outf32_1[i], &outf32_2[i], sinval[i], cosval[i]);
        }
#ifdef PF_COUNTER
        stopPFM;
        ACC_Result(&result);
