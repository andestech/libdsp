...
int main(void)
{
    ...
    {
        L1f1[inx] = atanf(n);
        q31_t q2 = riscv_dsp_atan_q31(convert_float_to_q31(n * tmp));
        L1f2[inx] = map_radian_q31_to_float(q2);
        inx++;
    }
    ...
    for (n = -RES_N; n < RES_N; n++)
    {
        ...
        q31_t q2 = riscv_dsp_atan_q31(convert_float_to_q31(n * tmp * tmp));
        L2f2[inx] = map_radian_q31_to_float(q2);
        inx++;
    }
    ...
    for (n = 1; n <= RES_N; n++)
    {
        L3f1[inx] = atanf(n);
        q31_t q2 = riscv_dsp_atan_q31(convert_float_to_q31(n * tmp));
        L3f2[inx] = map_radian_q31_to_float(q2);
        inx++;
    }
