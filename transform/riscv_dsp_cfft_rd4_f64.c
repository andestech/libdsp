...
/**
 * @brief Function to implement the f64 Radix-2 Complex FFT
 *        Functions
...
int32_t riscv_dsp_cfft_rd4_f64(float64_t *src, uint32_t m)
{
    register unsigned int j, k, n2, ie, ia1, ia2, ia3;
    float64_t tmp, c1, c2, c3, s1, s2, s3, xa, ya, xb, yb, xc, yc, xd, yd;
    float64_t *ptrs, *ptre, *ptr1, *ptr2, *ptr3;
    float64_t xt1, xt2, yt1, yt2;
    unsigned int n;
    ...
    while (ptrs != ptre)
    {
        ...
        ia2 = ia1 + ia1;
#if FFT_LOGN > RES_LOGN
        GET_COS_SIN_VALUES(ia2, c2, s2, p, f64);
#else
        GET_COS_SIN_VALUES(ia2, c2, s2, m, f64);
#endif /* FFT_LOGN > RES_LOGN */
        /* xb' = (xa - xb + xc - cd) * cos2 + (ya - yb + yc - yd) * sin2 */
        ...
        /* get cosine and sine values */
#if FFT_LOGN > RES_LOGN
        GET_COS_SIN_VALUES(ia1, c1, s1, p, f64);
#else
        GET_COS_SIN_VALUES(ia1, c1, s1, m, f64);
#endif /* FFT_LOGN > RES_LOGN */
        /* xc' = (xa + yb - xc - yd) * cos1 + (ya - xb - yc + xd) * sin1 */
        ...
        ia3 = ia2 + ia1;
#if FFT_LOGN > RES_LOGN
        GET_COS_SIN_VALUES(ia3, c3, s3, p, f64);
#else
        GET_COS_SIN_VALUES(ia3, c3, s3, m, f64);
#endif /* FFT_LOGN > RES_LOGN */
        /* xd' = (xa - yb - xc + yd) * cos3 + (ya + xb - yc - xd) * sin3 */
...
/**
 * @brief Function to implement the f64 Radix-2 Complex IFFT
 *        Functions
 * @param[in]       *src     points to the input block data, src[2 * k] and
 *                           src[2 * k + 1] contain the real and imaginary
 *                           parts of the k-th data.
...
int32_t riscv_dsp_cifft_rd4_f64(float64_t *src, uint32_t m)
{
    register unsigned int j, k, n2, ie, ia1, ia2, ia3;
    float64_t c1, c2, c3, s1, s2, s3, xa, ya, xb, yb, xc, yc, xd, yd;
    float64_t *ptrs, *ptre, *ptr1, *ptr2, *ptr3;
    float64_t xt1, xt2, yt1, yt2;
    unsigned int n;
