...
/**
 * @brief Function to implement the q31 Radix-2 Complex FFT
 *        Functions
...
int32_t riscv_dsp_cfft_rd4_q31(q31_t *src, uint32_t m)
{
    ...
    q31_t c1, c2, c3, s1, s2, s3;
    q31_t *ptrs, *ptre, *ptr1, *ptr2, *ptr3;
    q63_t xt1, xt2, yt1, yt2;
    q31_t xa, ya, xb, yb, xc, yc, xd, yd, tmp;
    unsigned int n = 1 << m;;
    ...
    while (ptrs != ptre)
    {
        ...
        ia2 = ia1 + ia1;
#if FFT_LOGN > RES_LOGN
        GET_COS_SIN_VALUES(ia2, c2, s2, p, q31);
#else
        GET_COS_SIN_VALUES(ia2, c2, s2, m, q31);
#endif /* FFT_LOGN > RES_LOGN */
        /* xb' = (xa - xb + xc - cd) * cos2 + (ya - yb + yc - yd) * sin2 */
        ptr1[0] = (c2 * xt1 + s2 * yt1) >> 32;
        /* yb' = (ya - yb + yc - yd) * cos2 - (xa - xb + xc - xd) * sin2 */
        ptr1[1] = (c2 * yt1 - s2 * xt1) >> 32;
        ...
        /* get cosine and sine values */
#if FFT_LOGN > RES_LOGN
        GET_COS_SIN_VALUES(ia1, c1, s1, p, q31);
#else
        GET_COS_SIN_VALUES(ia1, c1, s1, m, q31);
#endif /* FFT_LOGN > RES_LOGN */
        /* xc' = (xa + yb - xc - yd) * cos1 + (ya - xb - yc + xd) * sin1 */
        ptr2[0] = (c1 * xt1 + s1 * yt1) >> 32;
        /* yc' = (ya - xb - yc + xd) * cos1 + (xa + yb - xc - yd) * sin1 */
        ptr2[1] = (c1 * yt1 - s1 * xt1) >> 32;

        /* get cosine and sine values */
        ia3 = ia2 + ia1;
#if FFT_LOGN > RES_LOGN
        GET_COS_SIN_VALUES(ia3, c3, s3, p, q31);
#else
        GET_COS_SIN_VALUES(ia3, c3, s3, m, q31);
#endif /* FFT_LOGN > RES_LOGN */
        /* xd' = (xa - yb - xc + yd) * cos3 + (ya + xb - yc - xd) * sin3 */
        ptr3[0] = (c3 * xt2 + s3 * yt2) >> 32;
        /* yd' = (ya + xb - yc - xd) * cos3 + (xa - yb - xc + yd) * sin3 */
        ptr3[1] = (c3 * yt2 - s3 * xt2) >> 32;
        ptrs += 2;
    }
    ...
    for (k = (m >> 1) - 2; 0 != k; k--)
    {
        ...
        while (j)
        {
            LOOP_HINT_NO_HWLOOP();   //j is usually smaller than 6
            /* a = xa + ya * i, b = xb + yb * i, c = xc + yc * i,
             ** and d = xd + yd * i */
            ptr1 = ptr0 + n2;
            ptr2 = ptr1 + n2;
...
/**
 * @brief Function to implement the q31 Radix-2 Complex IFFT
 *        Functions
 * @param[in]       *src     points to the input block data, src[2 * k] and
 *                           src[2 * k + 1] contain the real and imaginary
 *                           parts of the k-th data.
...
{
    register unsigned int j, k, n2, ie, ia1, ia2, ia3;
    q31_t c1, c2, c3, s1, s2, s3, xa, ya, xb, yb, xc, yc, xd, yd;
    q31_t *ptrs, *ptre, *ptr1, *ptr2, *ptr3;
    q63_t xt1, xt2, yt1, yt2;
    unsigned int n;
...
#endif /* FFT_LOGN > RES_LOGN */
        /* xb' = (xa - xb + xc - cd) * cos2 + (ya - yb + yc - yd) * sin2 */
        ptr1[0] = (c2 * xt1 + s2 * yt1) >> 31;
        /* yb' = (ya - yb + yc - yd) * cos2 - (xa - xb + xc - xd) * sin2 */
        ptr1[1] = (c2 * yt1 - s2 * xt1) >> 31;
        xt1 = xc + yd;      /* xt1 = xa + yb - xc - yd */
        xt2 = xc - yd;      /* xt2 = xa - yb - xc + yd */
        yt1 = yc - xd;      /* yt1 = ya - xb - yc + xd */
        yt2 = yc + xd;      /* yt2 = ya + xb - yc - yd */
...
#endif /* FFT_LOGN > RES_LOGN */
        /* xc' = (xa + yb - xc - yd) * cos1 + (ya - xb - yc + xd) * sin1 */
        ptr2[0] = (c1 * xt1 + s1 * yt1) >> 31;
        /* yc' = (ya - xb - yc + xd) * cos1 + (xa + yb - xc - yd) * sin1 */
        ptr2[1] = (c1 * yt1 - s1 * xt1) >> 31;
...
#endif /* FFT_LOGN > RES_LOGN */
        /* xd' = (xa - yb - xc + yd) * cos3 + (ya + xb - yc - xd) * sin3 */
        ptr3[0] = (c3 * xt2 + s3 * yt2) >> 31;
        /* yd' = (ya + xb - yc - xd) * cos3 + (xa - yb - xc + yd) * sin3 */
        ptr3[1] = (c3 * yt2 - s3 * xt2) >> 31;
        ptrs += 2;
    }
    ...
    while (ptrs != ptre)
    {
        ...
        q31_t tmp_sum_r, tmp_sum_i;
        tmp_sum_r = NDS_ISA_KADDW(xa, xb);
        ptrs[0] = NDS_ISA_KSLRAW(tmp_sum_r, 1);
        tmp_sum_i = NDS_ISA_KADDW(ya, yb);
        ptrs[1] = -NDS_ISA_KSLRAW(tmp_sum_i, 1);

        tmp_sum_r = NDS_ISA_KSUBW(xa, xb);
        ptrs[2] = NDS_ISA_KSLRAW(tmp_sum_r, 1);
        tmp_sum_i = NDS_ISA_KSUBW(ya, yb);
        ptrs[3] = -NDS_ISA_KSLRAW(tmp_sum_i, 1);

        tmp_sum_r = NDS_ISA_KADDW(xc, yd);
        ptrs[4] = NDS_ISA_KSLRAW(tmp_sum_r, 1);
        tmp_sum_i = NDS_ISA_KSUBW(yc, xd);
        ptrs[5] = -NDS_ISA_KSLRAW(tmp_sum_i, 1);

        tmp_sum_r = NDS_ISA_KSUBW(xc, yd);
        ptrs[6] = NDS_ISA_KSLRAW(tmp_sum_r, 1);
        tmp_sum_i = NDS_ISA_KADDW(yc, xd);
        ptrs[7] = -NDS_ISA_KSLRAW(tmp_sum_i, 1);
...
int32_t riscv_dsp_cfft_rd4_q31_noscale(q31_t *src, uint32_t m)
{
...
int32_t riscv_dsp_cfft_rd4by2_q31(q31_t *src, uint32_t m)
{
    register unsigned int j, ia1, k;
    q31_t c1, s1, tmp_r1, tmp_i1, tmp_r2, tmp_i2;
    q63_t tmp_cr, tmp_ci;

    q31_t *ptrs_1 = NULL; 
    q31_t *ptrs_2 = NULL;
    q31_t *ptrs_1_tmp = NULL;
    q31_t *ptrs_2_tmp = NULL;
...
#if RES_LOGN <= 10
    q31_t tmp[1024*2];   // max support up to m = 10;
#else
    q31_t tmp[8192*2];   // max support up to m = 13;
#endif
#else
    q31_t * tmp = (q31_t*)NDSV_MALLOC(sizeof(q31_t) * 2 * 2 * n);
#endif
    ptrs_1 = &src[0];       
    ...
    for(j = 0 ; j < n ; j ++)
    {
        ...
        tmp_cr = (q63_t)(((q63_t) tmp_r2*c1 + (q63_t) tmp_i2*s1) >> 32);
        tmp_ci = (q63_t)(((q63_t) tmp_i2*c1 - (q63_t) tmp_r2*s1) >> 32);
        
        ptrs_2_tmp[2*j] = (q31_t) tmp_cr;
        ptrs_2_tmp[2*j+1] = (q31_t) tmp_ci;
    }

    riscv_dsp_cfft_rd4_q31(&ptrs_1_tmp[0], m-1);

    riscv_dsp_cfft_rd4_q31(&ptrs_2_tmp[0], m-1);
    ...
    for(j = 0 ; j < n ; j++)
    {
        tmp_cr = (q63_t) ptrs_1_tmp[2*j] << 1;
        tmp_ci = (q63_t) ptrs_1_tmp[2*j+1] << 1;
        src[2*k] =  (q31_t) NDS_ISA_SATS(tmp_cr, 32);
        src[2*k+1] = (q31_t) NDS_ISA_SATS(tmp_ci, 32);
        
        tmp_cr = (q63_t) ptrs_2_tmp[2*j] << 1;
        tmp_ci = (q63_t) ptrs_2_tmp[2*j+1] << 1;
        src[2*(k+1)] = (q31_t) NDS_ISA_SATS(tmp_cr, 32);
        src[2*(k+1)+1] = (q31_t) NDS_ISA_SATS(tmp_ci, 32);
        k += 2;
    }
...
#ifndef ENA_DSP_ISA_32
// this sub function only for pureC and RV64P for better performance
int32_t riscv_dsp_cfft_rd4by2_q31_small(q31_t *src, uint32_t m)
{
    register unsigned int j, ia1, k;
    q31_t c1, s1, tmp_r1, tmp_i1, tmp_r2, tmp_i2;
    q63_t tmp_cr, tmp_ci;
...
#endif // #ifndef ENA_DSP_ISA_32

// inverse fft radix4by2
int32_t riscv_dsp_cifft_rd4by2_q31(q31_t *src, uint32_t m)
{
    register unsigned int j, ia1, k;
    q31_t c1, s1, tmp_r1, tmp_i1, tmp_r2, tmp_i2;
    q63_t tmp_cr, tmp_ci;
...
#endif
#else
    q31_t * tmp = NDSV_MALLOC(sizeof(q31_t) * 2 * 2 * n);
#endif
    ptrs_1 = &src[0];
    ptrs_2 = &src[2*n];     // 1 complex = 2 real value
    ptrs_1_tmp = &tmp[0];
    ...
    for(j = 0 ; j < n ; j ++)
    {
        ...
        tmp_cr = (q63_t)(((q63_t) tmp_r2*c1 - (q63_t) tmp_i2*s1) >> 31);
        tmp_ci = (q63_t)(((q63_t) tmp_i2*c1 + (q63_t) tmp_r2*s1) >> 31);
        ptrs_2_tmp[2*j] = (q31_t) tmp_cr;
        ptrs_2_tmp[2*j+1] = (q31_t) tmp_ci;
    }
    riscv_dsp_cifft_rd4_q31(ptrs_1_tmp, m-1);

    riscv_dsp_cifft_rd4_q31(ptrs_2_tmp, m-1);
...
int32_t riscv_dsp_cifft_rd4by2_q31_small(q31_t *src, uint32_t m)
{
    register unsigned int j, ia1, k;
    q31_t c1, s1, tmp_r1, tmp_i1, tmp_r2, tmp_i2;
    q63_t tmp_cr, tmp_ci;
