...
void riscv_dsp_mfcc_f32(const riscv_dsp_mfcc_f32_t * instance, float32_t *src, float32_t *dst, float32_t *buf)
{
    ...
    uint32_t i;
    float32_t *coefs = instance->filter_coefs;

    /* Normalize */
    maxValue = riscv_dsp_absmax_val_f32(src, instance->fft_len);

    if (maxValue != 0.0f)
    {
        riscv_dsp_scale_f32(src, 1.0f / maxValue, src, instance->fft_len);
    }

    /* Multiply by window */
    riscv_dsp_mul_f32(src, instance->window_coefs, src, instance->fft_len);
    ...
    {
        result = riscv_dsp_dprod_f32(src + instance->filter_pos[i], coefs, instance->filter_len[i]);
    ...
    riscv_dsp_mat_mul_mxv_f32(instance->dct_coefs, buf, dst, instance->n_dct_out, instance->n_mel_filters);
}
