...
/**
 * @brief Function to implement the fp16 Real FFT Functions
...
int32_t riscv_dsp_rfft_f16(float16_t *src, uint32_t m)
{
    ...
    float16_t c, s, xt1, xt2, xt3, yt1, yt2, yt3;
    unsigned int n;
    n = 1 << m;
    ...
    {
#if FFT_LOGN > RES_LOGN
        GET_COS_SIN_VALUES(i, c, s, p, f16);
#else
        GET_COS_SIN_VALUES(i, c, s, m, f16);
#endif /* FFT_LOGN > RES_LOGN */
        xt1 = (float16_t)0.5 * (ptr[2 * i] + ptr[2 * j]);
        xt2 = (float16_t)0.5 * (ptr[2 * i] - ptr[2 * j]);
        yt1 = (float16_t)0.5 * (ptr[2 * i + 1] + ptr[2 * j + 1]);
        yt2 = (float16_t)0.5 * (ptr[2 * i + 1] - ptr[2 * j + 1]);
...
int32_t riscv_dsp_rifft_f16(float16_t *src, uint32_t m)
{
    ...
    xt1 = ptr[0];
    ptr[0] = (float16_t)0.5 * (xt1 + ptr[1]);
    ptr[1] = (float16_t)0.5 * (xt1 - ptr[1]);

    riscv_dsp_cifft_f16(src, m - 1);
