...
static inline float16_t dsp_atan1(float16_t x)
{
    unsigned ti = x * 100;
    float16_t lut1 = aractanlut[ti];
    float16_t lut2 = aractanlut[ti + 1];
...
float16_t riscv_dsp_atan2_f16(float16_t y, float16_t x)
{
    ...
    float16_t ret = (float16_t)atan2f(y, x);
    return ret;
#else
    //LUT for atan
    ...
    float16_t v;
    const float16_t absy = fabsf(y);
    const float16_t absx = fabsf(x);
