...
static inline q63_t atan1(const q31_t x)
{
    q63_t xi;
    unsigned ti = x >> 27;
    q63_t a = g[ti];
...
q31_t riscv_dsp_atan2_q31(q31_t y, q31_t x)
{
    if (y != 0)
    {
        ...
        t = (y1 / x1);
#else
        t = _dsp_div_u64_u32((uint64_t)y1, (uint32_t)x1);
#endif
        return (q31_t)(phi + atan1(t));
#else
        q31_t  ret;
        float32_t fy, fx, retf;

        fx = convert_q31_to_float(x);
        fy = convert_q31_to_float(y);

        retf = atan2f(fy, fx);
        ret = convert_float_to_q31(retf / PI);
        return ret;
#endif
    }
    else
